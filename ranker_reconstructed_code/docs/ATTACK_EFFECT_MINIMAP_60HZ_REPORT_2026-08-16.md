...
## 2026-08-18 전체 이펙트 렌더링 후속 감사

### 원본 렌더 순서와 위상 우선순위

- 프레임 합성 함수 `FUN_004d7790`의 순서를 다시 대조했다. 지형, 가시 유닛,
  수명주기 효과, 활성 유닛 효과, 맵 효과, 지형 장식, 정렬 큐, 안개, HUD 순서가
  재구성판과 일치한다.
- 정렬 함수 `FUN_0042a720`, 저번호 효과 렌더러 `FUN_004ed940`, 고번호 효과
  렌더러 `FUN_004f1a60`, 효과 정렬 함수 `FUN_004f28c0`을 전수 대조했다.
- 실제 차이는 `startup(0x02)`와 `impact(0x80)` 비트가 동시에 남는 프레임의
  위상 선택에서 확인됐다. 저번호 효과는 원본 `0x004ed947`처럼 impact 우선이고,
  고번호 효과는 공용 꼬리 `0x004f1dcc`처럼 startup 우선이다. 고번호 `0x63`만
  `0x004f1a71`의 impact 우선 예외다.
- 재구성판은 모든 효과에 impact 우선을 적용하고 있었다. 공용 위상 선택과
  특수 렌더러 `0x46`, `0x4b`, `0x56`, `0x59`, `0x62`를 원본 우선순위로
  수정했다. 이 변경은 렌더 프레임/그리기 모드 선택에만 적용되며 시뮬레이션,
  패킷, RNG 상태는 변경하지 않는다.

### 전체 리소스와 그리기 모드 감사

`tools/gameplay_parity/audit_effect_rendering_resources.py`로 원본 TRC를 직접
검사했다.

- `JW2_12`: 저번호 공격 효과 61개, 이미지 1,026개, 위상 프레임 참조 928개
- `JW2_11`: 고번호 스킬/생산 효과 46개, 이미지 1,516개, 위상 프레임 참조
  1,431개
- `JW2_10`/`JW2_07`: 지상 맵 효과 139개, 사용 아이콘 최댓값 119,
  원본 아이템 그룹 크기 121
- 모든 startup/active/impact 프레임 인덱스, 그리기 모드 0..9, 정렬 핸들러
  0..4가 유효했다. 원본이 의도한 레코드 간 참조는 저번호 11개, 고번호
  29개이며 범위 초과는 없었다.
- 감사 결과: `EFFECT_RENDERING_RESOURCE_AUDIT_PASS`

### 고기 수량 툴팁

- 원본 `FUN_004e9621`처럼 보이는 타일의 활성 맵 효과를 침입 리스트 순서로
  찾고, 원본 `FUN_004df316`처럼 효과 ID가 5 미만이면 카탈로그 이름 뒤에
  `=`와 맵 효과 raw `+0x30`의 실시간 수량을 붙인다.
- 출력 생성을 `BuildGameplayMapEffectTooltipText`로 고정했다. 카탈로그 이름을
  찾지 못한 경우 이전 툴팁 문자열이 남지 않도록 현재 효과의 대체 이름을
  사용한다.

### 직접 관련 검증

- `unit_effect_intrusive_iteration_regression`: 통과
  - 저번호 impact 우선, 고번호 startup 우선, `0x63` 예외, 특수 효과 ID의
    동시 플래그 우선순위를 확인했다.
  - 지상 고기 `이름=raw+0x30 수량`과 보유 고기 단계 경계
    100/101, 500/501, 1000/1001을 확인했다.
- `unit_effect_intrusive_iteration_regression kingdemon_afterimage_render`: 통과
- 기존 `audit_attack_effect_resources.py`: 통과
- 새 전체 이펙트 리소스 감사: 통과
- Release `ranker_rebuild`: 빌드 성공
