#!/usr/bin/env python3
"""Run every player UI producer/type pair through a completed spawn."""
...
def main() -> int:
    ...
    manifest = json.loads((tool_dir / "unit_production_batch_manifest.json")
    ...
    artifact_root = (root / "ranker_reconstructed_code" / "tools" /
                     ...
                     f"unit_production_parity_{stamp}")
    ...
    for number, batch in enumerate(batches, 1):
        ...
        for case in batch["cases"]:
            spawned = case["produced_unit_id"] in observed_types
            completed_count = (
                0, case["produced_unit_id"], 1) in completed_counts
            if not continuous_exact:
                ...
                verdict = "exact"
            row = {
                "kind": "unit_production", **case,
                "result": verdict,
                "spawned_type_seen": spawned,
                "completed_count_advanced": completed_count,
                "replay": batch["replay"],
