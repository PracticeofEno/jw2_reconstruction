#!/usr/bin/env python3
"""Compare every selector-three source/target definition pair."""
...
def main() -> int:
    ...
    manifest = json.loads((tool_dir / "definition_group_batch_manifest.json")
    ...
    artifact_root = (root / "ranker_reconstructed_code" / "tools" /
                     ...
                     f"definition_group_parity_{stamp}")
    ...
    for number, batch in enumerate(batches, 1):
        ...
        health_ranges = trace.get("semantic_coverage", {}).get(
            "unit_health_ranges", {})
        batch_failures = 0
        for case in batch["cases"]:
            states = ranges.get(str(case["source_slot"]), {}).get("states", [])
            target_health = health_ranges.get(str(case["target_slot"]), {})
            command_state_seen = any(value not in (0, 1) for value in states)
            health_changed = (
                target_health.get("min") != target_health.get("max") or
                target_health.get("max", case["target_initial_health"]) !=
                case["target_initial_health"])
            exercised = command_state_seen or health_changed
            verdict = ("exact" if continuous and exercised else
                       ("not_exercised" if continuous else "divergent"))
            row = {
                "kind": "definition_group_order", **case,
                "result": verdict,
                "command_state_seen": command_state_seen,
                "observed_source_states": states,
                "target_health_changed": health_changed,
                "target_health_range": target_health,
                "replay": batch["replay"],
