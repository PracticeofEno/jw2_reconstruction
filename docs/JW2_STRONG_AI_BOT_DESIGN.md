...
2026-09-07 관측 개정: 아래 기존 528개 특징의 순서는 유지하고, 자기 분대 재편성 경과시간 4개·수행 여부 4개·자기 교사 전략 파라미터 6개를 붙인 **542차원** 계약을 구현했다. 전략 값은 상대 내부 정보가 아니라 현재 자신에게 선택된 행동 방침이며 기본값은 교사 variant 0이다. 새 schema CRC는 `0xDA97FD92`, compact RLO 버전은 3(레코드 3,550바이트)이다. 기존 가중치·RLO는 명시적 변환 후 사용하고 기존 BC 통과·optimizer 상태를 자동 승계하지 않는다. 필드·이관·검증 명세는 [관측 보완 구현 기록](JW2_COMMANDER_CONTEXT_IMPROVEMENT_20260907.md)에 있다. γ/GAE λ는 각각 0.997/0.95를 기본값으로 유지하되 CLI·체크포인트 설정으로 저장·복원한다. 이 개정은 기존 승률·승격 게이트를 낮추지 않는다.

## 0. 이 문서를 읽는 법

후속 학습 대조에서는 γ 1.0·λ 0.98 후보가 동일 개발 조건에서 sampling 32→35승/48경기, argmax 43→44승/48경기를 기록했다. 건설 예약 만료 1회와 최종 200경기 sampling 검증이 남아 미승인 후보로 보존했다. 기본값을 바꾸거나 승격 게이트를 낮추지 않았으며, 조건·모델·검증 근거는 [후속 재점검 기록](JW2_COMMANDER_CONTEXT_RECOVERY_20260907.md)에 있다.
...
- **λ_k = max(0, 1 − k/300)** (k = PPO 반복). 셰이핑이 사라져도 빠른 승리 가산과 시간 할인은 남으므로 최종 학습 목표는 단순 승률과 같지 않다. 평가는 셰이핑 없는 승률을 주 지표로 본다. [2026-09-07 보상 점검](JW2_COMMANDER_CONTEXT_RECOVERY_20260907.md)에서 승수가 줄어도 할인 terminal 평균이 높아지는 사례를 확인했으며, γ 변경은 별도 후보의 실제 경기로 검증한다.
...
- argmax 48게임(전수) + 샘플링(τ=1, 게임 시드 100~149) 200게임(종족 50씩).
- 서로 다른 모델의 sampling 대조에는 평가 CLI `--policy-seed-base N`을 사용한다. 각 job에 `N+index`를 배정하고 native `-AIPOLICYSEED`의 실제 적용 로그를 검증한다. 옵션을 생략하면 기존처럼 모델 버전도 정책 RNG 초기화에 들어가므로 같은 게임 seed만으로 초기 정책 난수열이 같아지지 않는다. 명시적 seed는 모델 버전과 독립적인 초기 상태를 맞추지만, 행동과 조건부 mask가 달라진 뒤의 난수 소비까지 정렬하지는 않는다.
- 같은 정책 seed·실제 시작 위치·종족의 반복 실행은 독립 표본으로 중복 집계하지 않는다. Argmax에서는 seed만 바꾸고 시작 위치·종족을 유지해도 새로운 조건이 생기지 않는다. 개발 중 48조건 sampling 대조는 최종 200게임 평가를 대신하지 않는다.
